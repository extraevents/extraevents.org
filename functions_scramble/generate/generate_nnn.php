...
function generate_nnn($size, $lenght, $cut) {
    ...
    for ($i = 0; $i < 100; $i++) {
        ...
        if (!in_array($Layer . $Side, $layers_lock) and!in_array($Layer . $Side, $layers_lock_left)) {
            ...
            if (in_array($side_lock, array('R', 'L')) and!in_array($Side, array('R', 'L'))) {
                $side_lock = $Side;
                $layers_lock = array();
            }
            if (in_array($side_lock, array('U', 'D')) and!in_array($Side, array('U', 'D'))) {
                $side_lock = $Side;
                $layers_lock = array();
            }
            if (in_array($side_lock, array('B', 'F')) and!in_array($Side, array('F', 'B'))) {
                $side_lock = $Side;
                $layers_lock = array();
            }
