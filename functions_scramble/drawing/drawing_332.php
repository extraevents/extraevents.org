...
function drawing_332($scramble) {
    ...
    foreach (explode(" ", $scramble) as $move) {
        ...
        if ($move <> "" and isset($move[1]) and in_array($move[1], array("R", "L", "U", "D", "F", "B"))) {
            ...
            if (isset($move[3])) {
                if ($move[3] == " ") {
                    for ($l = 1; $l <= $move[0]; $l++) {
                        $CoorColor = Rotate($CoorColor, $circles, $l . $move[1], true);
                    }
                } elseif ($move[3] == "2") {
                    for ($l = 1; $l <= $move[0]; $l++) {
                        $CoorColor = Rotate($CoorColor, $circles, $l . $move[1], true);
                    ...
                    for ($l = 1; $l <= $move[0]; $l++) {
                        $CoorColor = Rotate($CoorColor, $circles, $l . $move[1], false);
                    }
                }
            } else {
                for ($l = 1; $l <= $move[0]; $l++) {
    ...
    $im = imagecreate($Border * 2 + $Ceil * $size * 4 + $D * 3, $Border * 2 + $Ceil * ($size * 3 - 1) + $D * 2);
