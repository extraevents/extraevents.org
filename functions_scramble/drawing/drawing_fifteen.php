...
function drawing_fifteen($scramble) {
    $Ceil = 100;
    $Border = 10;
    $D = 0;

    $Coor = array(
        0 => array(0 => 1, 1 => 5, 2 => 9, 3 => 13),
        1 => array(0 => 2, 1 => 6, 2 => 10, 3 => 14),
        2 => array(0 => 3, 1 => 7, 2 => 11, 3 => 15),
        3 => array(0 => 4, 1 => 8, 2 => 12, 3 => 0)
    );
...
//$scramble=str_replace('\r','',$scramble);
    ...
    for ($i = 0; $i < 4; $i++) {
        for ($j = 0; $j < 4; $j++) {
            ...
            if ($Coor[$i][$j]) {
                imagefttext($im, $Ceil / 2, 0, $dx + $Border + $Ceil * $i + $Ceil * ($Coor[$i][$j] > 9 ? 0.1 : .3), $dy + $Border + $Ceil * $j + $Ceil * .7, $color, 'fonts/arial.ttf', $Coor[$i][$j]);
            } else {
                imagefttext($im, $Ceil / 2, 0, $dx + $Border + $Ceil * $i + $Ceil * .35, $dy + $Border + $Ceil * $j + $Ceil * .75, $color, 'fonts/arial.ttf', '•');
            }
        }
    }
